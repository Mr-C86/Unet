...
if __name__ == '__main__':
    net = Unet(1, 1).cuda()
    net.load_state_dict(torch.load(r'params\params0.pth'))
    x = image.imread(r'E:\project\Unetdata\data\9.png')  # 原始数据
    img = x
    x = torch.Tensor(x)
    ...
    x1 = image.imread(r'E:\project\Unetdata\label\9.png')  # 标签
    x1 = torch.Tensor(x1)
    ...
    test_img = cv2.cvtColor(test_img, cv2.COLOR_GRAY2RGBA)#将图片由灰度图转化为四通道的图片
